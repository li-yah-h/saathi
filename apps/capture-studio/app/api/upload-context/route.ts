...
interface UploadContextBody {
  tile_id: string;
  context_names: string[]; // e.g. ["breakfast", "playground"]
}

/**
 * POST /api/upload-context
 *
 * Attaches one or more free-text context tags to a tile, creating any
 * tags that don't exist yet. This is what lets Diya's dynamic column
 * query "which tiles are relevant to the current context" rather than
 * relying only on raw embedding similarity.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Upsert the context tags themselves (name is unique).
    ...
    // Link the tile to each tag (idempotent — composite PK on join table).
